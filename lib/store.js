...
function pgDriver(pool) {
  ...
  const driver = {
    ...
    async listPremios(eventoId) {
      const res = await q(
        `SELECT p.id, p.orden, p.nombre, p.descripcion,
    ...
    async createPremio(eventoId, orden, nombre, descripcion) {
      const res = await q(
        `INSERT INTO premios (evento_id, orden, nombre, descripcion)
         VALUES ($1, $2, $3, $4) RETURNING *`,
        [eventoId, orden, nombre, descripcion]
      );
      return res.rows[0];
    },
