...
function pgDriver(pool) {
  ...
  const driver = {
    ...
    // Deja el sorteo a cero: sin premios ni resultados. Existe para limpiar un
    // ensayo hecho con cuentas reales, que "Borrar datos de prueba" no alcanza.
    // Resultados primero: si lo segundo fallara, los premios quedan pendientes
    // y sin ganador, que es un estado valido. Insignias y personas no se tocan.
    // Insignias de un ensayo hecho con cuentas reales. Solo con NADIE dentro:
    // antes de abrir no puede haber insignias de verdad, porque escanear exige
    // haber entrado. La condicion va dentro del mismo DELETE, asi que si
    // alguien entra entre la comprobacion y el borrado, no se borra nada.
    // scans_log no se toca: el rastro de cada escaneo sigue ahi.
    async contarDentro(eventoId) {
      const res = await q(
        `SELECT COUNT(*)::int AS n FROM asistentes_tickets
         WHERE evento_id = $1 AND estado = 'checkin'`,
        [eventoId]
      );
      return res.rows[0].n;
    ...
    async borrarInsigniasSinNadieDentro(eventoId) {
      const dentro = await q(
        `SELECT COUNT(*)::int AS n FROM asistentes_tickets
         WHERE evento_id = $1 AND estado = 'checkin'`,
        [eventoId]
      );
      if (dentro.rows[0].n > 0) return { dentro: dentro.rows[0].n, borradas: 0, detalle: [] };
      const detalle = await q(
        `SELECT t.codigo_ticket, COUNT(*)::int AS n
         FROM insignias i JOIN asistentes_tickets t ON t.id = i.ticket_id
         WHERE i.evento_id = $1
         GROUP BY t.codigo_ticket ORDER BY t.codigo_ticket`,
        [eventoId]
      );
      const res = await q(
        `DELETE FROM insignias
         WHERE evento_id = $1
           AND NOT EXISTS (SELECT 1 FROM asistentes_tickets
                           WHERE evento_id = $1 AND estado = 'checkin')`,
        [eventoId]
      );
      return { dentro: 0, borradas: res.rowCount, detalle: detalle.rows };
    },

    async borrarSorteo(eventoId) {
      const r = await q('DELETE FROM sorteo_resultados WHERE evento_id = $1', [eventoId]);
      const p = await q('DELETE FROM premios WHERE evento_id = $1', [eventoId]);
      return { resultados: r.rowCount, premios: p.rowCount };
    },
...
function memoryDriver() {
  ...
  return {
    ...
    async contarDentro(eventoId) {
      return db.asistentes.filter(a => a.evento_id === eventoId && a.estado === 'checkin').length;
    },

    async borrarInsigniasSinNadieDentro(eventoId) {
      const dentro = db.asistentes.filter(a => a.evento_id === eventoId && a.estado === 'checkin').length;
      if (dentro > 0) return { dentro, borradas: 0, detalle: [] };
      const propias = db.insignias.filter(i => i.evento_id === eventoId);
      const porCodigo = {};
      propias.forEach(i => {
        const t = db.asistentes.find(a => a.id === i.ticket_id) || {};
        porCodigo[t.codigo_ticket] = (porCodigo[t.codigo_ticket] || 0) + 1;
      });
      db.insignias = db.insignias.filter(i => i.evento_id !== eventoId);
      return {
        dentro: 0,
        borradas: propias.length,
        detalle: Object.keys(porCodigo).sort().map(c => ({ codigo_ticket: c, n: porCodigo[c] }))
      };
    },

    async borrarSorteo(eventoId) {
      const antesR = db.sorteoResultados.length;
      const antesP = db.premios.length;
      db.sorteoResultados = db.sorteoResultados.filter(r => r.evento_id !== eventoId);
      db.premios = db.premios.filter(p => p.evento_id !== eventoId);
      return { resultados: antesR - db.sorteoResultados.length, premios: antesP - db.premios.length };
    },
