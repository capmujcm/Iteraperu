...
const EMPRESA_UPDATABLE = ['nombre', 'rubro', 'emoji', 'color', 'descripcion',
  ...
  'instagram', 'facebook', 'tiktok', 'logo_mime', 'logo_datos', 'logo_actualizado_at',
  'password_hash', 'password_salt', 'password_algo', 'password_updated_at',
  'must_change_password', 'temp_password_expires_at',
...
function pgDriver(pool) {
  ...
  const driver = {
    ...
    async listEmpresasConToken(eventoId) {
      const res = await q(
        `SELECT id, evento_id, nombre, rubro, emoji, color, descripcion, stand,
                ...
                telefono, email, instagram, facebook, tiktok, created_at,
                must_change_password, temp_password_expires_at,
    ...
    async listPremios(eventoId) {
      const res = await q(
        `SELECT p.id, p.orden, p.nombre, p.descripcion, p.empresa_id,
         ...
         FROM premios p
         LEFT JOIN sorteo_resultados r ON r.premio_id = p.id
    ...
    async deletePremio(eventoId, id) {
      // No se borra un premio ya sorteado: seria borrar el resultado.
      const res = await q(
        `DELETE FROM premios
    ...
    // Participantes del bombo: quien tenga al menos una insignia y no haya
    // ganado ya. `boletos` es el numero de insignias, y cada insignia es un
    // boleto: quien visito cinco puestos tiene cinco veces mas probabilidad.
    async participantesSorteo(eventoId) {
      const res = await q(
    ...
    async listGanadores(eventoId) {
      const res = await q(
        `SELECT p.orden, p.nombre AS premio,
    ...
    // Borra SOLO lo marcado como prueba. Insignias, escaneos, sesiones y
    // resultados de sorteo asociados se van en cascada por las claves foraneas.
    async borrarDatosPrueba(eventoId) {
      ...
      return { personas: personas.rowCount, puestos: puestos.rowCount };
    },
...
function memoryDriver() {
  ...
  function vistaEmpresa(e, conToken) {
    ...
    ['id', 'evento_id', 'nombre', 'rubro', 'emoji', 'color', 'descripcion',
      ...
      'telefono', 'email', 'instagram', 'facebook', 'tiktok', 'created_at', 'must_change_password',
  ...
  return {
      ...
      return db.premios
        ...
        .map(p => {
          ...
          return Object.assign(clone(p), {
            ...
            sorteado: db.sorteoResultados.some(r => r.premio_id === p.id)
          });
        })
    ...
    async registrarResultadoSorteo(r) {
      const row = Object.assign({ id: uid(), created_at: new Date().toISOString() }, r);
    ...
    async borrarDatosPrueba(eventoId) {
      ...
      return { personas: personasFuera.length, puestos: puestosFuera.length };
    },
