...
function pgDriver(pool) {
  ...
  const driver = {
    ...
    // carteles. Nunca se expone en el catalogo publico.
    async listEmpresasConToken(eventoId) {
      const res = await q(
        'SELECT * FROM empresas WHERE evento_id = $1 ORDER BY nombre',
        [eventoId]
      );
      return res.rows;
    },
...
function memoryDriver() {
  ...
  return {
    ...
    async listEmpresasConToken(eventoId) {
      return db.empresas.filter(e => e.evento_id === eventoId).map(clone);
    },
