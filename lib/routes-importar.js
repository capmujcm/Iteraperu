...
module.exports = function registrarImportacion(fastify, opciones) {
  ...
  fastify.post('/api/soporte/empresas/importar', {
    ...
    for (const p of CATALOGO.puestos) {
      try {
        ...
        // La zona viaja con el resto: la consola arma con esto la hoja de
        // reparto, y una hoja de 72 filas sin la zona obliga a cruzarla a mano
        // con otro archivo para saber por donde empezar a repartir.
        creados.push({
  ...
  // ---------------------------------------------------------------------------
  // Reponer las claves de todos los puestos de una vez
  // ---------------------------------------------------------------------------
  // Las claves temporales se muestran UNA vez. Si la pestana se cierra antes de
  // descargarlas -o el import se corrio sin el archivo de contactos y no habia
  // a quien mandarselas- la unica salida era entrar puesto por puesto a la
  // ficha de cada uno y reponerla a mano. Setenta y dos veces.
  //
  // Por defecto solo toca a los puestos que NUNCA han entrado, es decir los que
  // siguen con el cambio de clave pendiente. Un puesto que ya definio su propia
  // contrasena esta usando el panel: reponersela lo echaria fuera y le cerraria
  // las sesiones sin que nadie se lo haya pedido. Para incluirlos hay que
  // decirlo explicitamente con `incluir_activos`.
  fastify.post('/api/soporte/empresas/claves', {
    preHandler: [requireOrganizador, rateLimit(3, 60000)]
  }, async (req, reply) => {
    const b = req.body || {};

    if (b.confirmar !== 'REPONER') {
      return reply.code(400).send({ error: 'Falta la confirmacion explicita.' });
    }

    const incluirActivos = b.incluir_activos === true;

    const todas = await store.listEmpresasConToken(eventoId);
    const objetivo = todas.filter(e => {
      if (e.evento_id !== eventoId) return false;   // punto 8, aunque ya filtre
      if (e.activo === false) return false;         // un puesto dado de baja no recibe claves
      if (!incluirActivos && !e.must_change_password) return false;
      return true;
    });

    const ahora = new Date().toISOString();
    const expira = new Date(Date.now() + auth.TEMP_EMPRESA_TTL_MS).toISOString();
    const accesos = [];
    const fallidos = [];
    ...
    for (const e of objetivo) {
      try {
        ...
        await store.updateEmpresa(e.id, {
          password_hash: hash, password_salt: salt, password_algo: algo,
          password_updated_at: ahora,
          must_change_password: true, temp_password_expires_at: expira,
          failed_login_count: 0, locked_until: null
        });
        // Si alguien tenia una sesion abierta con la clave anterior, se cierra.
        await store.revokeAllEmpresaSessions(e.id, null);
        ...
        // Uno que falle no detiene al resto: se anota el usuario, que no es un
        // dato personal, y el error se queda aqui.
        fallidos.push(e.usuario);
      }
    }

    // La bitacora guarda el recuento. Nunca las claves ni los telefonos.
    await registrarAccion(req, 'reponer_claves_puestos',
      `${accesos.length} puesto(s)` + (incluirActivos ? ' (incluidos los que ya entraron)' : '') +
    ...
    return reply.send({
      ...
      accesos,
      fallidos,
      omitidos_por_estar_en_uso: incluirActivos
        ? 0
        : todas.filter(e => e.activo !== false && !e.must_change_password).length,
      dias_validez: Math.round(auth.TEMP_EMPRESA_TTL_MS / 86400000),
      aviso: 'Las claves se muestran una sola vez. Descarga la hoja antes de cerrar.'
    });
  });
