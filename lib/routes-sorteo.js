...
module.exports = function registrarSorteo(fastify, opciones) {
  ...
  // ---------------------------------------------------------------------------
  // Empezar de cero (limpieza del ensayo)
  // ---------------------------------------------------------------------------
  // TEMPORAL: para dejar el evento limpio antes de abrir el dia 1 tras un ensayo
  // hecho con cuentas reales. Se retira en cuanto se use. En un solo paso:
  //   1. todos a "pendiente de ingreso" (como el reinicio diario);
  //   2. se borran todas las insignias (boletos);
  //   3. se borran todos los premios y resultados del sorteo.
  // NO toca personas, entradas, puestos (ni sus claves), staff ni registros.
  //
  // NO es el reinicio diario: el del domingo conserva las insignias del
  // sabado, y la app se lo promete a la gente. Por eso va aparte.
  //
  // Con mas de 20 personas dentro, el evento ya abrio y esto borraria datos de
  // verdad: se niega sin tocar nada. El ensayo tuvo un punado de cuentas.
  const MAX_DENTRO_PARA_LIMPIAR = 20;

  fastify.post('/api/soporte/empezar-de-cero', {
    preHandler: [requireOrganizador, rateLimit(3, 60000)]
  }, async (req, reply) => {
    const b = req.body || {};
    if (b.confirmar !== 'EMPEZAR DE CERO') {
      return reply.code(400).send({
        error: 'Falta la confirmación. Escribe exactamente EMPEZAR DE CERO.'
      });
    }

    const dentro = await store.contarDentro(eventoId);
    if (dentro > MAX_DENTRO_PARA_LIMPIAR) {
      return reply.code(409).send({
        error: `Hay ${dentro} personas dentro: el evento ya empezó. Esto es solo para ` +
          'limpiar el ensayo antes de abrir. No se borró nada.'
      });
    }

    // 1. Ingresos. Sin esto el paso 2 se negaria: con alguien dentro no se
    //    borran insignias.
    const reiniciados = await store.reiniciarIngresos(eventoId);

    // 2. Insignias. Si alguien entro justo entre el paso 1 y este, no se
    //    borra ninguna (la condicion va dentro del propio DELETE).
    const ins = await store.borrarInsigniasSinNadieDentro(eventoId);

    // 3. Sorteo, con su rastro leido antes de borrar.
    const ganadores = await store.listGanadores(eventoId);
    const sorteo = await store.borrarSorteo(eventoId);

    await registrarAccion(req, 'empezar_de_cero',
      `${reiniciados} ingreso(s) reiniciado(s); ` +
      `${ins.borradas} insignia(s) de ${ins.detalle.length} persona(s)` +
      (ins.detalle.length ? ' [' + ins.detalle.map(d => `${d.codigo_ticket} (${d.n})`).join(', ') + ']' : '') +
      `; ${sorteo.premios} premio(s) y ${sorteo.resultados} resultado(s)` +
      (ganadores.length ? ' [' + ganadores.map(g =>
        `${g.orden}º ${g.premio} → ${g.codigo_ticket || 'sin ticket'}`).join(', ') + ']' : ''));
    req.log.warn({ reiniciados, insignias: ins.borradas, sorteo }, 'evento puesto a cero tras el ensayo');

    return {
      success: true,
      reiniciados,
      insignias: ins.borradas,
      insignias_bloqueadas: ins.dentro > 0,
