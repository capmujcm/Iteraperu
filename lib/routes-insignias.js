...
// Usuario de acceso del puesto. Mismo criterio que el del staff.
...
// Color del puesto. Acaba dentro de un atributo style en el navegador de cada
// asistente, asi que se exige exactamente un color hexadecimal y nada mas.
...
module.exports = function registrarInsignias(fastify, opciones) {
  const { store, eventoId, rateLimit, porActor, requireAdmin, requireSession, exigirIngreso } = opciones;
  ...
  fastify.post('/api/insignias/scan', {
    // Primero la sesion y despues el limite, contado POR PERSONA. Por IP no
    // sirve: en el recinto cientos de celulares comparten la misma direccion y
    // 20 escaneos por minuto entre todos bloqueaba a gente legitima.
    preHandler: [
      requireSession(),
      rateLimit(ESCANEOS_POR_MINUTO, 60000, req => 'scan:' + req.persona.id)
    ]
  }, async (req, reply) => {
    const body = req.body || {};
  ...
  fastify.get('/api/insignias/mias', { preHandler: requireSession() }, async (req) => {
    ...
    // Si esta persona salio en el sorteo, su app se lo dice. Es SU resultado,
    // leido con SU sesion: no hay forma de pedir el de otro.
    const premio = await store.premioGanadoPor(eventoId, req.persona.id);
    return {
      success: true,
  ...
  fastify.post('/api/soporte/empresas', {
    ...
    // Identificador de acceso del responsable. Es DISTINTO del codigo impreso
    // bajo el QR: ese es publico y esta a la vista de todo el recinto.
    const usuario = normalizaUsuario(b.usuario);
    if (!usuario) {
      return reply.code(400).send({
        error: 'El usuario del puesto debe tener entre 3 y 40 caracteres: letras, números, punto, guion o guion bajo.'
      });
    }
    if (await store.findEmpresaByUsuario(eventoId, usuario)) {
      return reply.code(409).send({ error: 'Ya hay un puesto con ese usuario.' });
    }
    ...
    const empresa = await store.createEmpresa({
      ...
      color: colorValido(b.color, '#FF5A1F'),
      descripcion: recorta(b.descripcion, 1000),
      stand: recorta(b.stand, 30),
      condicion: recorta(b.condicion, 500),
      // Celular del responsable: sirve para entregarle sus credenciales por
      // WhatsApp sin que nadie tenga que transcribirlas a mano.
      telefono: recorta(b.telefono, 30),
      ...
      temp_password_expires_at: new Date(Date.now() + auth.TEMP_EMPRESA_TTL_MS).toISOString()
    });
  ...
  // ---------------------------------------------------------------------------
  // Organizador: corregir o dar de baja un puesto
  // ---------------------------------------------------------------------------
  // Hasta ahora solo el propio puesto podia editar su ficha. Si el puesto no
  // vino, o el nombre quedo mal escrito al crearlo, no habia forma de tocarlo.
  //
  // Lo que NO se puede cambiar ni siquiera desde aqui: el QR, el codigo impreso
  // y el usuario de acceso. Los carteles ya estan colgados.
  fastify.patch('/api/soporte/empresas/:id', {
    preHandler: [requireAdmin, rateLimit(60, 60000, porActor)]
  }, async (req, reply) => {
    // Punto 8: el puesto tiene que ser de ESTE evento. Si no, 404 y no 403.
    ...
    const patch = {};
    const campos = [
      ['nombre', 150], ['rubro', 60], ['emoji', 16], ['stand', 30],
      ['condicion', 500], ['descripcion', 1000], ['telefono', 30],
      ['responsable', 120], ['email', 150]
    ];
    campos.forEach(([campo, max]) => {
      if (b[campo] !== undefined) patch[campo] = recorta(b[campo], max);
    });
    if (b.color !== undefined) patch.color = colorValido(b.color, empresa.color || '#FF5A1F');
    ...
    const actualizada = await store.updateEmpresa(empresa.id, patch);

    const detalle = Object.keys(patch).map(k => (k === 'activo' ? (patch.activo ? 'activado' : 'desactivado') : k)).join(', ');
    await registrarAccion(req, 'editar_puesto', `${empresa.nombre}: ${detalle}`);

    // Misma proyeccion que el listado del organizador: con el token del QR y
    // sin hash ni bytes del logo.
    const vista = Object.assign({}, actualizada);
    delete vista.password_hash;
    delete vista.password_salt;
    delete vista.failed_login_count;
    delete vista.locked_until;
    vista.tiene_acceso = !!actualizada.password_hash;
    vista.qr_contenido = 'CFP|' + actualizada.qr_token;

    return reply.send({ success: true, empresa: vista });
  });
