...
const CREDENCIALES_INVALIDAS =
  'DNI o contraseña incorrectos. Si soporte te restableció la clave hace más ' +
  'de 30 minutos, pídela de nuevo en el Punto de Ayuda.';
...
const TEXTO_CONSENTIMIENTO =
  ...
  'para acreditación e ingreso al evento, para la dinámica de insignias y para ' +
  'participar en el sorteo. Acepto que, si resulto ganador, mi nombre y el ' +
  'código de mi entrada se muestren públicamente durante el sorteo del evento. ' +
  'Responsable: la empresa organizadora. Encargado del tratamiento: itera. ' +
  'Plazo: hasta 12 meses después del evento.';
...
const TEXTO_CONSENTIMIENTO_PUERTA =
  'Consentimiento verbal recogido en el Punto de Ayuda, con documento físico ' +
  'verificado por el staff, para acreditación e ingreso al evento.';
...
module.exports = function registrarAuth(fastify, { store, eventoId, rateLimit, porActor, requireAdmin }) {
  // ---------------------------------------------------------------------------
  // Guardia de sesion
  // ---------------------------------------------------------------------------
  ...
  function requireSession({ allowMustChange = false } = {}) {
    return async (req, reply) => {
      ...
      if (!sesion) {
        reply.code(401).send({ error: 'Tu sesión expiró. Vuelve a ingresar con tu DNI.' });
        return reply;
      }
  ...
  // ---------------------------------------------------------------------------
  // Limite por IP generoso a proposito: en el recinto muchos celulares comparten
  // IP (NAT de los operadores y wifi del local). Lo que frena el abuso es que
  // cada DNI solo se registra una vez.
  fastify.post('/api/auth/register', { preHandler: rateLimit(90, 60000) }, async (req, reply) => {
    const body = req.body || {};

    const dni = auth.normalizeDni(body.dni);
    if (!dni) {
      return reply.code(400).send({ error: 'Documento inválido. Revisa el número e intenta de nuevo.' });
    }
    ...
    if (body.acepta_privacidad !== true) {
      return reply.code(400).send({
        error: 'Debes aceptar la política de privacidad para registrarte.'
      });
    }
    ...
    if (existente) {
      ...
      return reply.code(409).send({
        error: 'Ya existe un registro con ese documento. Ingresa con tu contraseña.',
        ya_registrado: true
      });
    }

    const { hash, salt, algo } = await auth.hashPassword(pass.value);
  ...
  // Ingreso
  // ---------------------------------------------------------------------------
  // Mismo criterio que el registro: el tope por IP es alto porque la IP se
  // comparte; la fuerza bruta sobre una cuenta la corta el bloqueo por DNI
  // (8 intentos, 15 minutos) que viene mas abajo.
  fastify.post('/api/auth/login', { preHandler: rateLimit(150, 60000) }, async (req, reply) => {
    const body = req.body || {};
    const dni = auth.normalizeDni(body.dni);
    ...
    if (!(await auth.verifyPassword(password, persona.password_hash, persona.password_salt))) {
  ...
  fastify.post('/api/auth/change-password', {
    // Primero la sesion, y el limite se cuenta por persona, no por IP.
    preHandler: [
      requireSession({ allowMustChange: true }),
      rateLimit(10, 60000, req => 'clave:' + req.persona.id)
    ]
  }, async (req, reply) => {
    const body = req.body || {};
    ...
    if (!(await auth.verifyPassword(actual, persona.password_hash, persona.password_salt))) {
    ...
    if (await auth.verifyPassword(nueva.value, persona.password_hash, persona.password_salt)) {
      return reply.code(400).send({ error: 'La contraseña nueva debe ser distinta de la actual.' });
    }

    const { hash, salt, algo } = await auth.hashPassword(nueva.value);
  ...
  // ---------------------------------------------------------------------------
  // La persona completa o corrige sus propios datos
  // ---------------------------------------------------------------------------
  // Quien se registro en la puerta solo tiene un DNI guardado. Aqui pone su
  // nombre y lo que quiera anadir, desde su propio telefono.
  //
  // Solo toca SU ficha: el id sale de la sesion, nunca del cuerpo de la
  // peticion. Y solo estos seis campos: el estado del ticket, el check-in, la
  // contrasena y la prueba del consentimiento no se editan desde aqui.
  //
  // Corregir los propios datos no es una concesion: es el derecho de
  // rectificacion de la Ley 29733. Hasta ahora solo se podia ejercer por correo.
  const CAMPOS_PERFIL = ['nombre', 'apellido', 'celular', 'email', 'empresa', 'cargo'];

  fastify.patch('/api/auth/perfil', {
    preHandler: [requireSession(), rateLimit(20, 60000, req => 'perfil:' + req.persona.id)]
  }, async (req, reply) => {
    const body = req.body || {};
    const cambios = {};
    ...
    for (const campo of CAMPOS_PERFIL) {
      ...
      // Cadena vacia = borrar el dato. Es la unica forma que tiene la persona
      // de retirar un celular o un correo que ya no quiere que tengamos.
      const valor = body[campo] === '' ? null : texto(body[campo], LIMITES[campo]);
      cambios[campo] = valor;
    }

    if (!Object.keys(cambios).length) {
      return reply.code(400).send({ error: 'No enviaste ningún dato para actualizar.' });
    }

    // El nombre se puede rellenar y se puede corregir, pero no vaciar: es lo
    // que el staff lee en la puerta para saber a quien tiene delante.
    if (Object.prototype.hasOwnProperty.call(cambios, 'nombre') && !cambios.nombre) {
      return reply.code(400).send({ error: 'El nombre no puede quedar vacío.' });
    }

    if (cambios.email && !/^[^@\s]+@[^@\s.]+\.[^@\s]+$/.test(cambios.email)) {
      return reply.code(400).send({ error: 'Ese correo no parece válido.' });
    }

    const actualizada = await store.updateAttendee(req.persona.id, cambios);
    return { success: true, attendee: safeAttendee(actualizada) };
  });
  ...
  fastify.post('/api/soporte/reset-password', {
    preHandler: [requireAdmin, rateLimit(30, 60000, porActor)]
  }, async (req, reply) => {
    const body = req.body || {};
    const dni = auth.normalizeDni(body.dni);
    if (!dni) return reply.code(400).send({ error: 'Documento inválido.' });

    const persona = await store.findByDni(eventoId, dni);
    if (!persona) {
      return reply.code(404).send({ error: 'No hay ningún registro con ese documento.' });
    }
    ...
    await store.logPasswordReset(
      persona.id,
      (req.actor && req.actor.nombre) || 'Staff',
  ...
  fastify.post('/api/soporte/registro-rapido', {
    preHandler: [requireAdmin, rateLimit(60, 60000, porActor)]
  }, async (req, reply) => {
    const body = req.body || {};
    ...
    // En la puerta basta con el documento. El nombre es opcional: si el staff
    // tiene tiempo lo escribe, y si hay cola no se pide. La persona lo completa
    // sola desde su celular, que es donde el dato sale bien escrito y ademas lo
    // aporta su propio titular.
    const nombre = texto(body.nombre, LIMITES.nombre);
    ...
    const { hash, salt, algo } = await auth.hashPassword(auth.TEMP_PASSWORD);
    const ahora = new Date().toISOString();
    // Autor verificado por la sesion, no declarado en el cuerpo.
    const staffNombre = (req.actor && req.actor.nombre) || 'Staff';
    ...
    const datos = {
      ...
      password_updated_at: ahora,
      must_change_password: true,
      // Dura la jornada, no 30 minutos: la persona entra al recinto y abre su
      // cuenta horas despues, cuando quiere escanear su primer puesto.
      temp_password_expires_at: new Date(Date.now() + auth.TEMP_PASSWORD_PUERTA_TTL_MS).toISOString(),
      consentimiento: true,
      consentimiento_at: ahora,
    ...
    return reply.code(201).send({
      ...
      attendee: safeAttendee(persona),
      password_temporal: auth.TEMP_PASSWORD,
      minutos_validez: Math.round(auth.TEMP_PASSWORD_PUERTA_TTL_MS / 60000),
      horas_validez: Math.round(auth.TEMP_PASSWORD_PUERTA_TTL_MS / 3600000),
      checkin
    });
  });
