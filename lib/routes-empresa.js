...
// 4 MB descodificados. El tope anterior -400 KB- rechazaba fotos de logo salidas
// de un celular sin que el puesto supiera como reducirlas. Sigue habiendo tope:
// evita que alguien llene la base de datos con imagenes enormes, y el bodyLimit
// de las rutas de subida se calcula a partir de aqui.
...
// dias antes del evento. El valor vive en lib/auth.js: estaba copiado aqui y
// en otros dos archivos, y cambiarlo en uno solo no cambiaba nada.
const TEMP_EMPRESA_TTL_MS = auth.TEMP_EMPRESA_TTL_MS;

const CREDENCIALES_INVALIDAS =
  'Usuario o contraseña incorrectos. Si tu clave temporal caducó, ' +
  'pídela de nuevo a la organización.';

// Mismo criterio que para el staff: minúsculas, sin acentos ni espacios. Se
// escribe con prisa en un celular.
...
// Validacion de una imagen que llega en base64. La usan los dos caminos de
// subida -el del propio puesto y el de la organizacion- para que no puedan
// divergir: una sola puerta con una sola cerradura.
function revisarImagen(datos) {
  if (typeof datos !== 'string' || !datos) {
    return { error: 'No llegó ninguna imagen.', codigo: 400 };
  }

  // Se acepta con o sin prefijo data:. El prefijo se ignora por completo: el
  // tipo lo decide el contenido, no lo que diga el cliente.
  const limpio = datos.replace(/^data:[^;,]*;base64,/, '');
  if (!limpio || !/^[A-Za-z0-9+/=\s]+$/.test(limpio)) {
    return { error: 'La imagen no llegó en un formato válido.', codigo: 400 };
  }
  ...
  if (!buf.length) return { error: 'La imagen está vacía.', codigo: 400 };
  if (buf.length > LOGO_MAX_BYTES) {
    return {
      error: `La imagen pesa demasiado. El máximo son ${(LOGO_MAX_BYTES / (1024 * 1024)).toFixed(0)} MB.`,
      codigo: 413
    };
  }
...
module.exports = function registrarEmpresa(fastify, opciones) {
  const { store, eventoId, rateLimit, porActor, requireAdmin } = opciones;
  const zonaHoraria = opciones.zonaHoraria || 'America/Lima';
  ...
  // Ingreso
  // ---------------------------------------------------------------------------
  // Los puestos entran desde el recinto, con la misma IP compartida que todo
  // el mundo. El bloqueo por usuario (8 intentos) es lo que frena la fuerza bruta.
  fastify.post('/api/negocio/login', { preHandler: rateLimit(60, 60000) }, async (req, reply) => {
    const b = req.body || {};
    // Se entra con el USUARIO del puesto, no con el código impreso bajo su QR:
    // ese código está a la vista de todo el recinto y usarlo como identificador
    // de acceso regalaba la lista de usuarios válidos.
    ...
    const empresa = await store.findEmpresaByUsuario(eventoId, usuario);
    ...
    if (!(await auth.verifyPassword(password, completa.password_hash, completa.password_salt))) {
  ...
  fastify.post('/api/negocio/change-password', {
    ...
    if (!(await auth.verifyPassword(String(b.password_actual || ''), e.password_hash, e.password_salt))) {
    ...
    if (await auth.verifyPassword(nueva.value, e.password_hash, e.password_salt)) {
  ...
  fastify.patch('/api/negocio/perfil', { preHandler: requireEmpresa() }, async (req, reply) => {
    ...
    const campos = [
      ...
      ['responsable', 120], ['telefono', 30], ['email', 150],
      ['instagram', 80], ['facebook', 80], ['tiktok', 80], ['whatsapp', 30],
      ['ruc', 20], ['emoji', 16]
    ];
  ...
  // @fastify/multipart por un unico endpoint de subida
  // (checklist punto 7: preferir la solucion sin dependencia).
  fastify.post('/api/negocio/logo', {
    preHandler: [rateLimit(6, 60000), requireEmpresa()],
    // base64 infla ~33%, mas el resto del JSON.
    bodyLimit: Math.ceil(LOGO_MAX_BYTES * 1.4) + 4096
  }, async (req, reply) => {
    const revisado = revisarImagen((req.body || {}).datos);
    if (revisado.error) return reply.code(revisado.codigo).send({ error: revisado.error });

    const mime = revisado.mime;
    const buf = revisado.buf;
  ...
  // Mismo logo, subido por la organización en lugar del propio puesto. Hace
  // falta cuando el responsable no puede o no sabe hacerlo, y es lo que usa el
  // generador de datos de prueba.
  fastify.post('/api/soporte/empresas/:id/logo', {
    preHandler: [requireAdmin, rateLimit(30, 60000)],
    bodyLimit: Math.ceil(LOGO_MAX_BYTES * 1.4) + 4096
  }, async (req, reply) => {
    ...
    const actualizada = await store.updateEmpresa(empresa.id, {
      logo_mime: revisado.mime,
      logo_datos: revisado.buf,
      logo_actualizado_at: new Date().toISOString()
    });

    req.log.info({ empresaId: empresa.id, bytes: revisado.buf.length }, 'logo subido por la organizacion');
    return reply.send({ success: true, empresa: vistaPropia(actualizada) });
  });
  ...
  fastify.get('/api/negocio/stats', { preHandler: requireEmpresa() }, async (req) => {
    const stats = await store.statsEmpresa(req.empresa.id, zonaHoraria);
    return {
      success: true,
  ...
  fastify.post('/api/soporte/empresas/:id/acceso', {
    ...
    const { hash, salt, algo } = await auth.hashPassword(clave);
