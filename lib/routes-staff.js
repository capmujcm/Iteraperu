...
module.exports = function registrarStaff(fastify, opciones) {
  ...
  function vistaUsuario(u) {
    if (!u) return null;
    // `locked_until` viaja para que la gestion de usuarios pueda mostrar quien
    // esta bloqueado por intentos fallidos y ofrecer desbloquearlo. Es estado
    // operativo, no un dato personal.
    const bloqueado = !!(u.locked_until && new Date(u.locked_until).getTime() > Date.now());
    return {
      id: u.id, usuario: u.usuario, nombre: u.nombre, rol: u.rol,
      activo: u.activo, must_change_password: u.must_change_password,
      bloqueado, locked_until: bloqueado ? u.locked_until : null,
  ...
  // Ingreso
  // ---------------------------------------------------------------------------
  // Todo el staff entra a la vez a la misma hora, desde el mismo wifi. El
  // bloqueo por usuario (8 intentos) es lo que frena la fuerza bruta.
  fastify.post('/api/staff/login', { preHandler: rateLimit(60, 60000) }, async (req, reply) => {
    const b = req.body || {};
    ...
    if (!(await auth.verifyPassword(password, u.password_hash, u.password_salt))) {
  ...
  fastify.post('/api/staff/change-password', {
    ...
    const b = req.body || {};
    if (!(await auth.verifyPassword(String(b.password_actual || ''), u.password_hash, u.password_salt))) {
    ...
    if (await auth.verifyPassword(nueva.value, u.password_hash, u.password_salt)) {
  ...
  // Quitar el bloqueo por intentos fallidos.
  //
  // Ocho fallos sobre un usuario lo dejan fuera 15 minutos. En la puerta, con
  // cola delante, 15 minutos es una eternidad; y como el bloqueo es por usuario
  // -no por quien lo intenta- cualquiera que conozca el nombre de alguien del
  // staff puede dejarlo fuera con ocho peticiones. El organizador lo levanta
  // sin tener que tocarle la contrasena.
  fastify.post('/api/staff/usuarios/:id/desbloquear', {
    preHandler: [requireOrganizador(), rateLimit(30, 60000)]
  }, async (req, reply) => {
    // Punto 8: tiene que ser un usuario de ESTE evento. 404 y no 403, para no
    // confirmar que el id existe en otro sitio.
    const u = await store.findUsuarioStaffById(req.params.id);
    if (!u || u.evento_id !== eventoId) {
      return reply.code(404).send({ error: 'Usuario no encontrado.' });
    }
    ...
    return reply.send({ success: true, usuario: u.usuario });
  });
